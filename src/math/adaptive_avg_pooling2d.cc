...
template <typename T>
void adaptive_avg_pooling2d_block_impl(
  T *input,
  T *output,
      ...
      for (int64_t ox = 0; ox < output_width; ++ox) {
        ...
        for (int64_t iy = s_iy; iy < s_iy + kernel_height; ++iy) {
          for (int64_t ix = s_ix; ix < s_ix + kernel_width; ++ix) {
            sum_val += input[c * input_height * input_width + iy * input_width + ix];
          }
        }

        output[c * output_height * output_width + oy * output_width + ox] = sum_val / T(kernel_height * kernel_width);
      }
    }
  }
}
...
template <typename T>
void adaptive_avg_pooling2d_impl(
  ...
  for (int64_t i = 0; i < num_threads; ++i) {
    ...
    eigen_device->enqueue_with_barrier(
        &barrier,
        &adaptive_avg_pooling2d_block_impl<T>,
        x,
        y,
        start_channel,
        end_channel,
...
void adaptive_avg_pooling2d(const Tensor &x, Tensor &y) {
  if (x.element_type().is<float>()) {
    adaptive_avg_pooling2d_impl<float>(
      x.eigen_device().get(),
      x.data<float>(),
      x.shape(),
      y.data<float>(),
