...
void instance_norm2d_float_block_impl(
...

  {
    ...
    for (; c < col_limit; c += 4) {
      sum = vaddq_f32(sum, vld1q_f32(xptr + c));
    }
...

  {
    ...
    variance += sum[0] + sum[1] + sum[2] + sum[3];
#endif
...
void instance_norm2d(Tensor &input, const Tensor &scale, const Tensor &shift, float eps, Tensor &output) {
