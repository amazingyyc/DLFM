...
void TinyDown::torch_name_scope(std::string name) {
  torch_name_scope_ = name;

  op_->torch_name_scope(name + TORCH_NAME_SCOPE_SEP + "op");
}

std::vector<Module> TinyDown::sub_modules() {
  return { op_ };
}
...
TinyUp::TinyUp(int64_t in_channel, int64_t out_channel) {
  ...
  up_ = conv_tranpose2d(in_channel, out_channel, {3, 3}, {2, 2}, {1, 1}, {1, 1});
...
void TinyUp::torch_name_scope(std::string name) {
  torch_name_scope_ = name;
...
std::vector<Module> TinyUp::sub_modules() {
  return { up_, conv_ };
}
...
TinyUNet::TinyUNet(int64_t in_channels, int64_t out_channels) {
  input_ = sequential({
    conv2d(in_channels, 32, {3, 3}, {1, 1}, {1, 1}),
    relu(true)
  });

  down1_ = std::make_shared<TinyDown>(32, 64);
  down2_ = std::make_shared<TinyDown>(64, 128);
  down3_ = std::make_shared<TinyDown>(128, 256);
  down4_ = std::make_shared<TinyDown>(256, 512);

  up1_  = std::make_shared<TinyUp>(512, 256);
  up2_  = std::make_shared<TinyUp>(256, 128);
  up3_  = std::make_shared<TinyUp>(128, 64);
  up4_  = std::make_shared<TinyUp>(64, 32);
...
void TinyUNet::torch_name_scope(std::string name) {
  torch_name_scope_ = name;
...
std::vector<Module> TinyUNet::sub_modules() {
