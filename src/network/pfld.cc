...
PFLDInference::PFLDInference() {
  ADD_SUB_MODULE(conv1, conv2d, 3, 64, 3, 2, 1, 1, 1, false);
  ADD_SUB_MODULE(bn1, batch_norm2d, 64);

  ADD_SUB_MODULE(conv2, conv2d, 64, 64, 3, 1, 1, 1, 1, false);
  ...
  ADD_SUB_MODULE(conv8, conv2d, 32, 128, 7, 1, 0, 1, 1, true);
...
Tensor PFLDInference::forward(Tensor x) {
  x = ((*bn1)((*conv1)(x))).relu(true); // [64, 56, 56]
  x = ((*bn2)((*conv2)(x))).relu(true); // [64, 56, 56]
